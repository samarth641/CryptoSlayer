...
// Colors - Crypto themed
const BITCOIN_GOLD = { r: 0xf7, g: 0x93, b: 0x1a }; // Bitcoin orange/gold
const ETHEREUM_BLUE = { r: 0x62, g: 0x7e, b: 0xea }; // Ethereum blue
const DOGECOIN_GREEN = { r: 0xc2, g: 0xa0, b: 0x3d }; // Dogecoin green
const CARDANO_PURPLE = { r: 0x00, g: 0x3a, b: 0xad }; // Cardano purple
const BOMB_RED = { r: 0xff, g: 0x00, b: 0x00 }; // Bomb red
const BOMB_DARK = { r: 0x33, g: 0x33, b: 0x33 }; // Bomb dark gray

const cryptoColors = [BITCOIN_GOLD, ETHEREUM_BLUE, DOGECOIN_GREEN, CARDANO_PURPLE];
const bombColors = [BOMB_RED, BOMB_DARK];
const allColors = [...cryptoColors, ...bombColors];
...
// Image loading system for crypto tokens
const cryptoImages = {};
const imagePromises = [];
...
// Load crypto token images from local files
imagePromises.push(loadCryptoImage('./images/btc.png', 'bitcoin'));
imagePromises.push(loadCryptoImage('./images/eth.png', 'ethereum'));
imagePromises.push(loadCryptoImage('./images/doge.png', 'dogecoin'));
imagePromises.push(loadCryptoImage('./images/cardano.png', 'cardano'));
imagePromises.push(loadCryptoImage('./images/bomb.png', 'bomb'));
...
// Crypto token models
function makeBitcoinModel({ scale=1 }) {
	// Bitcoin - circular coin shape
	const segments = 12;
	const vertices = [];
	const polys = [];
	
	// Create circular top and bottom faces
	for (let i = 0; i < segments; i++) {
		const angle = (i / segments) * TAU;
		const nextAngle = ((i + 1) / segments) * TAU;
		
		// Top face
		vertices.push({ x: 0, y: 0, z: scale });
		vertices.push({ x: Math.cos(angle) * scale, y: Math.sin(angle) * scale, z: scale });
		vertices.push({ x: Math.cos(nextAngle) * scale, y: Math.sin(nextAngle) * scale, z: scale });
		...
		// Top face triangle
		polys.push({ vIndexes: [i * 6, i * 6 + 1, i * 6 + 2] });
		// Bottom face triangle
		polys.push({ vIndexes: [i * 6 + 5, i * 6 + 4, i * 6 + 3] });
		// Side quad
		polys.push({ vIndexes: [i * 6 + 1, i * 6 + 4, i * 6 + 5, i * 6 + 2] });
	}
	
	return { vertices, polys };
}

function makeEthereumModel({ scale=1 }) {
	// Ethereum - hexagonal shape
	const vertices = [
		// Top hexagon
		{ x: 0, y: scale, z: scale },
		{ x: scale * 0.866, y: scale * 0.5, z: scale },
		{ x: scale * 0.866, y: -scale * 0.5, z: scale },
		{ x: 0, y: -scale, z: scale },
		{ x: -scale * 0.866, y: -scale * 0.5, z: scale },
		{ x: -scale * 0.866, y: scale * 0.5, z: scale },
	...
	const polys = [
		// Top face
		{ vIndexes: [0, 1, 2, 3, 4, 5] },
		// Bottom face
		{ vIndexes: [11, 10, 9, 8, 7, 6] },
		// Side faces
		{ vIndexes: [0, 6, 7, 1] },
		{ vIndexes: [1, 7, 8, 2] },
		{ vIndexes: [2, 8, 9, 3] },
...
function makeBombModel({ scale=1 }) {
	// Bomb - spherical with fuse
	const segments = 8;
	const vertices = [];
	const polys = [];
	...
	// Add fuse
	const fuseLength = scale * 1.5;
	vertices.push({ x: 0, y: 0, z: scale });
	vertices.push({ x: 0, y: 0, z: scale + fuseLength });
	
	// Fuse line
	polys.push({ vIndexes: [vertices.length - 2, vertices.length - 1] });
...
const getTarget = (() => {
	...
	function getTargetOfStyle(color, wireframe) {
		...
		if (!target) {
			// Use 3D models with crypto-themed colors
			let model;
			if (color === BITCOIN_GOLD) {
				model = makeBitcoinModel({ scale: targetRadius });
			} else if (color === ETHEREUM_BLUE) {
				model = makeEthereumModel({ scale: targetRadius });
			} else if (bombColors.includes(color)) {
				model = makeBombModel({ scale: targetRadius });
			} else {
				// Default to Bitcoin model for other crypto colors
				model = makeBitcoinModel({ scale: targetRadius });
			}
	...
	// Target Parameters
	// --------------------------------
	let color = pickOne(cryptoColors);
	let wireframe = false;
	let health = 1;
	let maxHealth = 3;
	let isBomb = false;
	...
	else if (Math.random() < 0.15) { // 15% chance for bomb
		color = pickOne(bombColors);
		isBomb = true;
		health = 1;
		maxHealth = 1;
	}
...
// Bomb explosion function (currently unused but kept for potential future features)
function explodeBomb(bomb, targets, bombIndex) {
	const explosionRadius = 150;
	const bombX = bomb.x;
	const bombY = bomb.y;
	const bombZ = bomb.z;
	
	// Find and destroy nearby targets
	for (let i = targets.length - 1; i >= 0; i--) {
		if (i === bombIndex) continue; // Skip the bomb itself
		...
		if (distance <= explosionRadius) {
			...
			createBurst(target, 1.5);
			sparkBurst(target.projected.x, target.projected.y, 12, 15);
			targets.splice(i, 1);
			returnTarget(target);
		}
	}
...
function renderScoreHud() {
	...
	cubeCountNode.innerText = `TOKENS SMASHED: ${state.game.cubeCount}`;
}
...
function tick(width, height, simTime, simSpeed, lag) {
			...
			if (distance <= targetHitRadius) {
				...
				if (!target.hit) {
					...
					if (pointerSpeedScaled > minPointerSpeed) {
						...
						if (target.isBomb) {
							// Bomb hit - GAME OVER!
							sparkBurst(hitX, hitY, 30, sparkSpeed * 3);
							// Remove the bomb
							targets.splice(i, 1);
							returnTarget(target);
							// End the game
	...
	PERF_START('updateParticles');
	backgroundParticles.forEach((particle, index) => {
		particle.update();
		if (particle.life <= 0) {
			backgroundParticles[index] = new Particle();
		}
	});
	PERF_END('updateParticles');
	...
	// Skip 3D rendering for targets (we're using 2D images)
	allVertices.length = 0;
	allPolys.length = 0;
	allShadowVertices.length = 0;
	allShadowPolys.length = 0;
	// Only add fragments, not targets
	targets.forEach(entity => {
		// Skip targets - they use 2D images
	});
...
function draw(ctx, width, height, viewScale) {
	...
	// 2D Images and 3D Polys
	// ---------------
	ctx.lineJoin = 'bevel';

	// Draw 2D crypto token images
	PERF_START('drawCryptoImages');
	targets.forEach((target, index) => {
		const size = targetRadius * 2;
		const time = state.game.time * 0.001; // Convert to seconds
		
		// Use 3D projection for proper positioning
		const focalLength = cameraDistance * sceneScale;
		const depth = focalLength / (cameraDistance - target.z);
		...
		ctx.save();
		ctx.translate(x, y);
		ctx.rotate(target.rotateZ);
		
		// Determine which image to use
		let imageName = 'bitcoin';
		if (target.color === ETHEREUM_BLUE) imageName = 'ethereum';
		else if (target.color === DOGECOIN_GREEN) imageName = 'dogecoin';
		else if (target.color === CARDANO_PURPLE) imageName = 'cardano';
		else if (bombColors.includes(target.color)) imageName = 'bomb';
		
		// Add pulsing glow effect
		const pulseIntensity = 0.3 + 0.2 * Math.sin(time * 3 + index * 0.5);
		const glowSize = size * (1 + pulseIntensity * 0.1);
		
		// Draw outer glow
		ctx.shadowColor = colorToHex(target.color);
		ctx.shadowBlur = 15 + pulseIntensity * 10;
		ctx.fillStyle = colorToHex(target.color);
		ctx.globalAlpha = 0.3 + pulseIntensity * 0.2;
		ctx.beginPath();
		ctx.arc(0, 0, glowSize / 2, 0, TAU);
		ctx.fill();
		
		// Draw main circle with enhanced glow
		ctx.shadowBlur = 8;
		ctx.globalAlpha = 1;
		ctx.fillStyle = colorToHex(target.color);
		ctx.beginPath();
		...
		// Add animated border
		const borderPulse = 0.5 + 0.5 * Math.sin(time * 4 + index * 0.3);
		ctx.strokeStyle = `rgba(255, 255, 255, ${0.3 + borderPulse * 0.4})`;
		ctx.lineWidth = 2 + borderPulse;
		ctx.stroke();
		
		// Draw PNG image if available
		if (cryptoImages[imageName]) {
			// Add slight rotation animation
			ctx.save();
			ctx.rotate(time * 0.1 + index * 0.2);
			ctx.drawImage(cryptoImages[imageName], -size / 2, -size / 2, size, size);
			ctx.restore();
		} else {
			// Fallback: draw text symbol with glow
			...
			let symbol = '₿';
			if (target.color === ETHEREUM_BLUE) symbol = 'Ξ';
			else if (target.color === DOGECOIN_GREEN) symbol = 'Ð';
			else if (target.color === CARDANO_PURPLE) symbol = '₳';
			else if (bombColors.includes(target.color)) symbol = '💣';
			
			ctx.fillText(symbol, 0, 0);
		}
		
		// Enhanced glow effect for bombs - make them more dangerous looking
		if (bombColors.includes(target.color)) {
			const bombPulse = 0.5 + 0.5 * Math.sin(time * 8);
			const dangerPulse = 0.3 + 0.7 * Math.sin(time * 12);
			
			// Outer danger ring
			ctx.shadowColor = '#ff0000';
			ctx.shadowBlur = 25 + bombPulse * 20;
			ctx.fillStyle = `rgba(255, 0, 0, ${0.2 + dangerPulse * 0.4})`;
			ctx.beginPath();
			ctx.arc(0, 0, size / 2 + 15 + bombPulse * 8, 0, TAU);
			ctx.fill();
			
			// Inner warning glow
			ctx.shadowBlur = 15 + bombPulse * 10;
			ctx.fillStyle = `rgba(255, 100, 0, ${0.5 + bombPulse * 0.3})`;
			ctx.beginPath();
			ctx.arc(0, 0, size / 2 + 5 + bombPulse * 3, 0, TAU);
			ctx.fill();
			...
			ctx.textAlign = 'center';
			ctx.textBaseline = 'middle';
			ctx.fillText('DANGER!', 0, -size / 2 - 20);
			ctx.restore();
		}
	...
	PERF_END('drawCryptoImages');

	PERF_START('drawShadows');
	// Only draw shadows for fragments, not targets (targets use 2D images)
	...
	// Only draw fragments, not targets (targets use 2D images)
	allPolys.forEach(p => {
	...
	PERF_START('drawBackgroundParticles');
	backgroundParticles.forEach(particle => {
		particle.draw(ctx);
	});
	PERF_END('drawBackgroundParticles');
...
function setupCanvas() {
	...
	// Wait for crypto images to load
	Promise.all(imagePromises).then(() => {
		console.log('Crypto images loaded successfully');
		console.log('Available images:', Object.keys(cryptoImages));
	}).catch(error => {
		console.error('Error loading crypto images:', error);
	});
